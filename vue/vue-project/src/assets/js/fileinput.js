...
(function ($) {
    ...
    var isIE = function (ver) {
        ...
        tActionDelete = '<button type="button" class="kv-file-remove {removeClass}" ' +
            'title="{removeTitle}"{dataUrl}{dataKey} deletecid="{pid}">{removeIcon}</button>\n',
        ...
        resetFileStack: function () {
            ...
            self.$preview.find(PREVIEW_FRAMES).each(function () {
                ...
                if (file !== undefined) {
					...
					//按业务需求，修改代码，上传后依然使用之前生成的previewInitId变量
					//by chenchao 20160709 modified
                    /**$thumb.attr({
                        'id': 'uploaded-' + uniqId(),
                        'data-fileindex': '-1'
                    });**/
...
//            });
        ...
        renderFileFooter: function (caption, width,previewId) {
        ...
        initPreviewDeletes: function () {
            ...
            self.$preview.find('.kv-file-remove').each(function () {
				...
				//init extraData add by mu start
                extraData = isEmpty(config) || isEmpty(config.extra) ? deleteExtraData : config.extra;
                //init extraData add by mu end
                settings = $.extend({
                    url: vUrl,
                    type: 'POST',
                    dataType: 'json',
        ...
        previewDefault: function (file, previewId, isDisabled) {
            ...
            var self = this, data = objUrl.createObjectURL(file), $obj = $('#' + previewId),
                ...
                footer = self.renderFileFooter(file.name, config.width,previewId),
        ...
        previewFile: function (file, theFile, previewId, data) {
            ...
            var self = this, cat = self.parseFileType(file), caption = self.slug(file.name), content, strText,
                ...
                footer = self.renderFileFooter(caption, config.width,previewId), modal = '',
                ind = previewId.slice(previewId.lastIndexOf('-') + 1);
